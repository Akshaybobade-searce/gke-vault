...
kubectl create serviceaccount vault-auth -n atalantis

# Create the RBAC rolebinding for token review for the vault-auth service account
kubectl apply -n atalantis -f - <<EOH
---
...
SECRET_NAME="$(kubectl get serviceaccount vault-auth -n atalantis \
  -o go-template='{{ (index .secrets 0).name }}')"

# Get the actual token reviewer account
TR_ACCOUNT_TOKEN="$(kubectl get secret "${SECRET_NAME}" -n atalantis \
